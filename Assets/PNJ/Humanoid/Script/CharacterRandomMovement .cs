...
public class CharacterRandomMovement : MonoBehaviour
{
    ...
    public float wanderRadius = 20f; // Rayon autour du PNJ pour choisir la prochaine destination
    public float movementSpeed = 0.5f; // Vitesse personnalisée du PNJ (ajustable dans l'inspecteur)
    ...
    void Update()
{
    ...
    void MoveToRandomPosition()
    {
        ...
        randomDirection += transform.position; // Position autour du PNJ
        randomDirection.y = transform.position.y; // Garde la même hauteur

        NavMeshHit hit;
        if (NavMesh.SamplePosition(randomDirection, out hit, 5.0f, NavMesh.AllAreas))
        {
            agent.SetDestination(hit.position);
            animator.SetBool("isWalking", true); // ✅ Animation de marche
        }
    }
